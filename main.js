...
a.forEach(el => el.addEventListener('click', (event) => {
...
let widthSlide = slider.clientWidth;
...
let sliderIndicatorElements = document.querySelectorAll('.slider-indicator-element');
...
function tabs(event) {
    ...
    if (window.matchMedia("(min-width: 767px)").matches) { 
        ...
        if (objects.eventCurrentTarget.classList.contains('tab-active')) {
            closeTab(objects);
            if (objects.tabPosition == 'main-tab') { mainRemoveIcons(objects) };
            if (objects.eventCurrentTarget.classList.contains('testimoanials-title')) { footerSliderArrowsRemove(objects); };
        } else {
            openTab(objects);
            if (objects.tabPosition == 'main-tab') { mainAddIcons(objects) };
            if (objects.eventCurrentTarget.classList.contains('testimoanials-title')) { footerSliderArrowsAdd(objects); };
        }
    }
}
...
function getObjects(event) {
    ...
    return { eventCurrentTarget, id, tabPosition, order, tabsItems, tabsContent, closeArrows, icons, activeIcon, activeCloseArrow, activeContetnItem, footerSliderArrows };
}


function mainTabsTogglePc({ tabsItems, tabsContent, icons, eventCurrentTarget, activeIcon, order, activeContetnItem }){
...
function openTab ( { tabsItems, tabsContent, closeArrows, eventCurrentTarget, activeCloseArrow, activeContetnItem} ) {
    tabsItems.forEach(el => el.classList.remove('tab-active'));
...
function footerSliderArrowsAdd( { footerSliderArrows } ) {
    setTimeout(() => { footerSliderArrows.classList.add('slider-arrow-active')}, 290);
}
...
footerRightArrow.addEventListener('click', () => {
    ...
    footerSlider.style.transform = `translateX(${footerPosition * footerSlider.offsetWidth}px)`;
})
